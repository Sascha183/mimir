...
import {
  JUMP_PROGRAMS,
  findJumpProgram,
} from '../lib/cpu/jump-programs';
import type { CpuState } from '../lib/cpu/types';
import CpuDiagram from './cpu/CpuDiagram';

/**
 * JumpInstructionsCycle — lesson-24 scene.
 *
 * Two pre-loaded loop programs (JMP and JMPR variants). The user picks
 * which to run, then watches R1 increment forever as the loop iterates.
 * IAR's value visibly snaps backwards each time a jump fires.
 */

const DEFAULT_RATE_MS = 500;
const MIN_RATE_MS = 200;
const MAX_RATE_MS = 2000;
const STORAGE_KEY = 'hciw:jump-instructions:state';
...
function defaultPersisted(): PersistedState {
  return { rateMs: DEFAULT_RATE_MS, programKey: JUMP_PROGRAMS[0].key };
}
  ...
  try {
    ...
    return {
      ...
      programKey:
        ...
        JUMP_PROGRAMS.some((p) => p.key === parsed.programKey)
          ? parsed.programKey
          : JUMP_PROGRAMS[0].key,
    };
  } catch {
...
export default function JumpInstructionsCycle() {
  const [programKey, setProgramKey] = useState(JUMP_PROGRAMS[0].key);
  const [cpuState, setCpuState] = useState<CpuState>(() =>
    JUMP_PROGRAMS[0].build(),
  );
  ...
  const program = useMemo(() => findJumpProgram(programKey), [programKey]);
  ...
  useEffect(() => {
    ...
    setCpuState(findJumpProgram(loaded.programKey).build());
    setHydrated(true);
  }, []);
  ...
  function pickProgram(key: string) {
    setRunning(false);
    setProgramKey(key);
    setCpuState(findJumpProgram(key).build());
    setCycleStartIAR(0);
    setIterations(0);
  }

  function advanceOnce(prev: CpuState): CpuState {
    const r = buildRecipeForIR(prev.registers.IR, prev.flags);
    const next = executeStep(prev, r.steps[prev.stepIdx]);
    if (next.stepIdx === 0) {
      setCycleStartIAR(next.registers.IAR);
      // If we just completed a cycle that ended with IAR landing on the
      // loop head, increment the iteration counter. We use loopHeadAddress
      // as the marker.
      if (next.registers.IAR === program.loopHeadAddress) {
  ...
  function step() {
    setCpuState((prev) => advanceOnce(prev));
  }
  ...
  function stepInstruction() {
    setCpuState((prev) => {
      ...
      if (s.registers.IAR === program.loopHeadAddress) {
        setIterations((it) => it + 1);
      }
      return s;
    });
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- advanceOnce
    // captures program/recipe via closure; we re-run on running/rateMs/program changes.
  }, [running, rateMs, program]);
      ...
      <div className="mt-5 rounded-xl border border-apple-border bg-apple-surface p-4">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <span className="font-mono text-[11px] text-apple-text-secondary">
            iterations:{' '}
            <output
              aria-label="loop iteration count"
              className="font-semibold text-apple-text"
            >
              {iterations}
            </output>
          </span>
